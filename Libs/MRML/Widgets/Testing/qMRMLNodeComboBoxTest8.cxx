...
int qMRMLNodeComboBoxTest8( int argc, char * argv [] )
{
  ...
  if (startingActions + 1 != actionsPlusOne)
    {
    std::cout << "After adding a second new user action that conflicts with a default one, new number of actions "
	      << actionsPlusOne << " != " << startingActions << " + 1" << std::endl;
    return EXIT_FAILURE;
    }
  else
    {
    std::cout << "After adding a second new user action that conflicts with a default one, new number of actions = " << actionsPlusOne << std::endl;
    }
  ...
  if (startingActions + 1 != actionsPlusOne)
    {
    std::cout << "After adding a third new user action with duplicate text, new number of actions "
	      << actionsPlusOne << " != " << startingActions << " + 1" << std::endl;
    return EXIT_FAILURE;
    }
  else
