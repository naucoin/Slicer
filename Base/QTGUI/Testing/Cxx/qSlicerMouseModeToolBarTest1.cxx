...
int qSlicerMouseModeToolBarTest1(int argc, char * argv[] )
{
  ...
  if (selectionNode)
    ...
    activeActionText = mouseToolBar.activeActionText();
    std::cout << "After setting selection node activeplace node class name to " << selectionNode->GetActivePlaceNodeClassName() << ", mouse tool bar active action text = " << qPrintable(activeActionText) << std::endl;
    ...
    activeActionText = mouseToolBar.activeActionText();
    std::cout << "After setting selection node active place node class name to " << selectionNode->GetActivePlaceNodeClassName() << ", mouse tool bar active action text = " << qPrintable(activeActionText) << std::endl;
    ...
    if (activeActionText.compare(QString("")) != 0)
      {
      std::cerr << "Error! Expected active action text of '', got '" << qPrintable(activeActionText) << "'" << std::endl;
      return EXIT_FAILURE;
      }
    }
