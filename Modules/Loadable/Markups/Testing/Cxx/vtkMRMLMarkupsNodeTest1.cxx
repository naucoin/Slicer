...
int vtkMRMLMarkupsNodeTest1(int , char * [] )
{
  ...
  for (int m = -1; m < 3; m++)
	...
	else { std::cout << "\t" << m << " doesn't exist, good!" << std::endl; }
    }
  ...
  for (int p = -1; p < 3; p++)
    ...
    for (int m = -1; m < 3; m++)
	  ...
	  else { std::cout << "Points exists in markup: success on point " << p << " in markup " <<  m << std::endl; }
      }
    }
  ...
  if (id.compare("vtkMRMLScalarVolumeNode1") != 0)
    {
    std::cerr << "Failed to get back expected id of vtkMRMLScalarVolumeNode1, got '" << id.c_str() << "'" << std::endl;
    return EXIT_FAILURE;
    }
  else
    {
  ...
  if (p1.GetX() != p0[0] ||
      ...
      p1.GetZ() != p0[2])
    {
    std::cerr << "Failed to get back markup 2 point 1 via vector, expected " << p0[0] << "," << p0[1] << "," << p0[2] << ", but got " << p1.GetX() << "," << p1.GetY() << "," << p1.GetZ() << std::endl;
    return EXIT_FAILURE;
    }
  else
    {
  ...
  if (p3[0] != p0[0] ||
      ...
      p3[2] != p0[2])
    {
    std::cerr << "Failed to get back markup 2 point 1 via array, expected " << p0[0] << "," << p0[1] << "," << p0[2] << ", but got " << p3[0] << "," << p3[1] << "," << p3[2] << std::endl;
    return EXIT_FAILURE;
    }
  else
    {
